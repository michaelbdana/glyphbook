...
3. ✔ **Core model & editor** — DONE. Schema + block/mark extensions (scene break, small caps/mono/sans), no-data-loss codecs (`validation.ts`), dashboard + full book/chapter CRUD (start/duplicate/delete, Book Details dialog, add/delete chapters), project save/load + timed snapshots + per-book snapshot export, drag-and-drop chapter reordering & cross-section moves (`reorder.ts`, unit-tested), recent-first library ordering, autosave with Saving/Saved indicators. **Next: writing tools (4).**
4. ✔ **Writing tools** — DONE. Find & Replace (book-wide, case option, per-chapter match list, Replace All, pure tested core), **Book Goal** (target words + due date + writing days → words/day needed) and **Writing Habit** (daily target, streak & words-today from an autosaved per-day word log), **Sprint timer**, **Smart quotes** (whole-book curly-quote conversion, tested), **Editor settings** (display-only font/size/line-height/paragraph spacing, persisted), **spellcheck** toggle. All surfaced via a Tools panel from the Writing status bar. **Next: book structure (5).**
5. ✔ **Book structure** — DONE (core). Standard auto pages on new books (Title/Copyright/TOC, kinds `title|copyright|toc`), preset page layouts per section (Dedication, Epigraph, Blurbs, Foreword, Preface, Introduction, Prologue, Epilogue, Afterword, Acknowledgements, About the Author, Also By), per-chapter **Chapter Options** (Include In All/eBook/Print/None, Begin On auto/left/right, hide heading/page number/header-footer/TOC, smaller title, invert text), **Parts** and **Volumes** with keep-children delete and chapter assignment, volume→part nesting. Scene breaks are in-place block elements; a dedicated "Scene 1..N" split view is deferred (see next milestone). **Next: themes & formatting (6).**
6. ✔ **Themes & formatting** — DONE (core). Theme-as-data model (`theme.ts`: body/heading fonts from an offline font catalog, sizes, line height, justify, paragraph start, first-line drop caps, scene-break ornament, trim-size catalog, margins, layout priority, notes placement for print/eBook, page-number location, running header, large-print override) with 6 preset themes + merge/effective-theme helpers. Deterministic **theme → print CSS compiler** (`themeCss.ts`, unit-tested) now drives the print pipeline (paged.js output reflects the book's theme: page size, running headers, page numbers, drop caps). Formatting screen: theme library grid with live preview + **Customize Theme** builder; stored per book (`themeName` + overrides) and preserved by the validator. **Next: imaging (7).**
7. ✔ **Imaging** — DONE (core). Native image picker (JPG/PNG/WebP/GIF → data-URL, embedded in the model for portability), TipTap **image block** with React node view: inline editing of alignment, width %, caption, alt text, wrap-text, separate-page (print), remove; rendered in device preview and the print pipeline (figures/captions/float). **Full Page Image** chapter kind with its own editor (choose/replace/remove + alt). Image-size **calculator** dialog (full-bleed vs in-margin pixel math, unit-tested) on the Formatting screen. **Next: previews (8).**
8. ✔ **Previews** — DONE. Shared reflowable eBook renderer (`previewDoc.ts`) drives device previews from the same content/theme as export: chapter/section headings, paragraphs, images/captions, scene-break ornaments, full-page images. New `BookPreview` component with a device catalog (Kindle Paperwhite/Oasis, Nook Glowlight, Kobo Forma, iPhone, Galaxy, iPad, Kindle Fire) and a **Print** view (compiled print CSS, true-to-export page shape); wired into the Writing preview (current chapter) and the Formatting rail (whole book). **Next: exporters (9).**
9. ✔ **Exporters** — DONE. **Print PDF** via the paged.js window driven by the active book (compiled theme CSS → `printToPDF`, written to the exports folder). **EPUB3** generator (`electron/exporters/epub.ts`): valid container (mimetype-first ZIP verified), OPF metadata, per-chapter XHTML + stylesheet, EPUB nav + NCX TOC honoring per-chapter include/hide options. **DOCX** generator (headings + justified, first-line-indented paragraphs). Export buttons on the Formatting screen (PDF/ePub/DOCX) with "Reveal in folder" dialogs; headless export smoke tests for all three formats. **Next: polish & packaging (10).**
10. ✔ **Polish & packaging** — DONE (core). electron-builder config (appId, icons, NSIS/DMG/AppImage+deb targets) with `npm run dist:*`; generated app icon; packaged Linux dir verified running the full print/export pipeline from inside the asar. **First-run flow** (empty library + Start/Upload/Load Sample) and a working **.docx import** (Heading-1 chapters via mammoth → book model with auto front matter). Conformance map: `docs/conformance.md`. Remaining parity gaps are documented there (boxesets, scene-list UI, link icons, mobi, sorting UI, export diagnostics, page-balancing tuning).
